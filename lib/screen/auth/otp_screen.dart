...
class VerificaionScreen extends StatefulWidget {
  final String number;
  final String verify;
  const VerificaionScreen({
    Key? key,
...
class _VerificaionScreenState extends State<VerificaionScreen> {
  ...
  numberVerify() {
    ...
    authendicationApi.numberVerificaion(number).then((value) {
  ...
  numVerification() {
    ...
    if (otpGet == myotp) {
      ...
      } else if (widget.verify == 'register') {
        setState(() {
          isLoading = false;
        });
        Navigator.push(
